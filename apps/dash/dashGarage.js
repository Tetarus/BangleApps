tcBack.replaceWith(() => {
  ...
  if (UI.ntid && face[0].bar) {
    ...
    UI.ntid = 0;
    face[0].bar();
  } else if (!euc.dash.info.get.makr || !ew.def.dash.slot || !require("Storage").readJSON("logDaySlot" + ew.def.dash.slot + ".json", 1)) face.go("clock", 0);
...
face[0] = {
  ...
  bar: function () {
    ...
    let target = { InmotionV10: "im", InmotionV11: "im", InmotionV12: "im", Begode: "bg", NinebotS: "nb", NinebotZ: "nb", Veteran: "vt", NinebotE: "nb", Kingsong: "ks" };
  ...
  tap: function (no) {
    ...
    if (this.prevSlot == no) {
      ...
      if (require("Storage").read("eucSlot" + no + ".json")) euc.dash = require("Storage").readJSON("eucSlot" + no + ".json", 1);
