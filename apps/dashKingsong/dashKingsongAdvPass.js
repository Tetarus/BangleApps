    ...
    if (i == 2) {
      ...
      UIc.bar._bar = (i) => {
        if (i == 6) {
          ...
          UI.btn.ntfy(1, 1, 0, "_bar", 6, "PASSWORD", "REMOVED", 15, 4);
          w.gfx.flip();
          setTimeout(() => {
...
face[0].act = () => {
  if (face[0].page == "password?") {
    if (euc.dash.opt.lock.pass == face[0].pass) {
      ...
      UI.btn.ntfy(1, 1, 0, "_bar", 6, "CODE", "ACCEPTED", 15, 4);
      w.gfx.flip();
      setTimeout(() => {
        face[0].opt();
      }, 1000);
      ...
      face[0].pass = "";
      UI.btn.ntfy(1, 1.5, 0, "_bar", 6, "CODE", "IS WRONG", 15, 13);
      w.gfx.flip();
    }
    ...
    if (!face[0].passone) {
      ...
      face[0].pass = "";
      UI.btn.ntfy(1, 1, 0, "_bar", 6, "RE-ENTER", "TO VERIFY", 15, 4);
      w.gfx.flip();
    } else {
      if (face[0].pass == face[0].passone) {
        if (face[0].page == "change password") {
          ...
          UI.btn.ntfy(1, 1, 0, "_bar", 6, "CODE", "CHANGED", 15, 4);
          w.gfx.flip();
        } else {
          euc.dash.opt.lock.passBck = euc.dash.opt.lock.pass;
          euc.dash.opt.lock.pass = face[0].pass;
          euc.wri("setPass");
          UI.btn.ntfy(1, 1, 0, "_bar", 6, "CODE", "APPLIED", 15, 4);
          w.gfx.flip();
        }
        ...
        buzzer.nav([20, 100, 200]);
        UI.btn.ntfy(1, 1, 0, "_bar", 6, "MISMATCH", "TRY AGAIN", 15, 13);
        w.gfx.flip();
        face[0].pass = "";
        face[0].passone = "";
      }
    ...
    euc.dash.opt.lock.pass = face[0].pass;
    euc.wri("setPassSend");
    UI.btn.ntfy(1, 3, 0, "_bar", 6, "PLEASE", "WAIT", 15, 1);
    w.gfx.flip();
    setTimeout(() => {
      if (euc.temp.pass) {
        buzzer.nav([20, 100, 200]);
        ...
        UI.btn.ntfy(1, 3, 0, "_bar", 6, "CODE", "ACCEPTED", 15, 4);
        w.gfx.flip();
        setTimeout(() => {
          euc.dash.auto.onC.pass = 1;
