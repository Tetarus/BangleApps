...
UIc.main._2x2 = (i) => {
  if (i == 1) {
    buzzer.nav(buzzer.buzz.ok);
    euc.dash.auto.onC.HL++;
    if (3 < euc.dash.auto.onC.HL) euc.dash.auto.onC.HL = 0;
    let val = ["NA", "ON", "OFF", "AUTO"];
    UI.btn.c2l("main", "_2x2", 1, "LIGHT", val[euc.dash.auto.onC.HL], 15, euc.dash.auto.onC.HL ? (euc.dash.auto.onC.HL != 2 ? 12 : 1) : 0);
    UI.btn.ntfy(0, 2, 0, "_ind", 1, euc.dash.auto.onC.HL ? "SET LIGHT " + val[euc.dash.auto.onC.HL] : "NO ACTION", "", 0, 15);
    w.gfx.flip();
  } else if (i == 2) {
    buzzer.nav(buzzer.buzz.ok);
    euc.dash.auto.onC.led++;
    if (2 < euc.dash.auto.onC.led) euc.dash.auto.onC.led = 0;
    UI.btn.c2l("main", "_2x2", 2, "LED", "RIDE", 15, euc.dash.auto.onC.led ? (euc.dash.auto.onC.led == 1 ? 12 : 1) : 0);
    UI.btn.ntfy(0, 2, 0, "_ind", 1, euc.dash.auto.onC.led ? (euc.dash.auto.onC.led == 1 ? "ENABLE" : "DISABLE") : "NO ACTION", "", 0, 15);
    w.gfx.flip();
  }
};
UIc.bar._2x2 = (i) => {
  if (i == 3) {
    buzzer.nav(buzzer.buzz.ok);
    euc.dash.auto.onC.lift++;
    if (2 < euc.dash.auto.onC.lift) euc.dash.auto.onC.lift = 0;
    UI.btn.c2l("main", "_2x2", 3, "SENSOR", "LIFT", 15, euc.dash.auto.onC.lift ? (euc.dash.auto.onC.lift == 1 ? 12 : 1) : 0);
    UI.btn.ntfy(0, 2, 0, "_ind", 1, euc.dash.auto.onC.lift ? (euc.dash.auto.onC.lift == 1 ? "ENABLE" : "DISABLE") : "NO ACTION", "", 0, 15);
    w.gfx.flip();
  } else if (i == 4) {
    buzzer.nav(buzzer.buzz.ok);
    euc.dash.auto.onC.talk++;
    if (2 < euc.dash.auto.onC.talk) euc.dash.auto.onC.talk = 0;
    UI.btn.c2l("main", "_2x2", 4, "VOICE", "MODE", 15, euc.dash.auto.onC.talk ? (euc.dash.auto.onC.talk == 2 ? 12 : 1) : 0);
    UI.btn.ntfy(0, 2, 0, "_ind", 1, euc.dash.auto.onC.talk ? (euc.dash.auto.onC.talk == 1 ? "ENABLE" : "DISABLE") : "NO ACTION", "", 0, 15);
    w.gfx.flip();
  }
};
