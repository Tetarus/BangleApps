...
euc.temp.checksum = function (packet) {
  ...
  const receivedCRC32 = new DataView(packet).getUint32(packet.length - 4, false);
...
euc.temp.voltToPercent = function (volt) {
  ...
  const voltTable = new Float32Array([124.45, 122.63, 122.08, 121.87, 121.22, 119.75, 118.07, 116.77, 115.46, 114.29, 113.12, 111.92, 110.63, 109.36, 107.95, 106.37, 104.74, 103.3, 100.47, 97.49, 94.5, 90.31, 87]);
...
euc.temp.liveParse = function (inc) {
  ...
  switch (euc.dash.opt.bat.pack) {
    case 24: // not Patton
      ...
      break;
    case 30: // Patton
      euc.dash.live.bat = euc.temp.voltToPercent(euc.dash.live.volt);
...
euc.temp.inpk = function (event) {
  ...
  if (euc.temp.isValidStartPacket(inc)) euc.temp.tot = E.toUint8Array(inc);
  ...
  if (euc.temp.tot.buffer.length === expectedLength) euc.temp.processPacket(euc.temp.tot.buffer);
