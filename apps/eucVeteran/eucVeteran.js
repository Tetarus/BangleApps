// Command function to handle different operations
euc.cmd = function (command) {
  const commands = {
...
// Function to calculate and verify the checksum of a packet
function verifyChecksum(packet) {
  const FWVer = (packet[28] << 8) | packet[29];

  if (FWVer < 3012) {
    ew.is.bt === 2 && euc.dbg && console.log("Firmware does not support checksum. FWVer:", FWVer);
    return true;
  }

  ew.is.bt === 2 && euc.dbg && console.log("Checksum verification");
  ...
  return calculatedCRC32 === receivedCRC32;
}

// Function to convert voltage to percentage
function convertVoltageToPercentage(volt) {
  ...
  while (low <= high) {
    ...
    if (voltTable[mid] < volt) high = mid - 1;
    else low = mid + 1;
  }

  const interpolatedPercent = percentTable[high] + ((volt - voltTable[high]) * (percentTable[high + 1] - percentTable[high])) / (voltTable[high + 1] - voltTable[high]);
  return Math.round(interpolatedPercent);
}

// Function to parse the incoming data packet
euc.temp.liveParse = function (inc) {
  const dataView = new DataView(inc);
  euc.is.alert = 0;

  // Voltage and Battery Calculation
  euc.dash.live.volt = dataView.getUint16(4) / 100;

  switch (euc.dash.opt.bat.pack) {
    case 24:
      euc.dash.live.bat = euc.dash.live.volt > 100.2 ? 100 : euc.dash.live.volt > 81.6 ? Math.round((euc.dash.live.volt - 80.7) / 0.195) : euc.dash.live.volt > 79.35 ? Math.round((euc.dash.live.volt - 79.35) / 0.4875) : 0;
      break;
    case 30:
      euc.dash.live.bat = convertVoltageToPercentage(euc.dash.live.volt);
      break;
    case 36:
      euc.dash.live.bat = euc.dash.live.volt > 150.3 ? 100 : euc.dash.live.volt > 122.4 ? Math.round((euc.dash.live.volt - 119.7) / 0.306) : euc.dash.live.volt > 115.2 ? Math.round((euc.dash.live.volt - 115.2) / 0.81) : 0;
      break;
    default:
      euc.dash.live.bat = 1; // Default for unknown wheels
  }
  ...
  if (euc.dash.alrt.amp.hapt.en && euc.dash.alrt.amp.cc === 2) {
    const alertAdjustment = euc.dash.alrt.amp.hapt.hi <= euc.dash.live.amp ? euc.dash.live.amp - euc.dash.alrt.amp.hapt.hi : -(euc.dash.live.amp - euc.dash.alrt.amp.hapt.low);
    euc.is.alert += 1 + Math.round(alertAdjustment / euc.dash.alrt.amp.hapt.step);
  }
  ...
  const modelVer = Math.floor(ver / 1000);
  euc.dash.info.get.firm = `${modelVer}.${Math.floor((ver % 1000) / 100)}.${ver % 100}`;
  ...
  if (euc.dash.alrt.pwr) euc.is.alert++;
  ...
  if (euc.dash.alrt.pwm.hapt.en && (euc.dash.alrt.pwr || euc.dash.alrt.pwm.hapt.hi <= euc.dash.live.pwm)) {
    ...
    const buzzPattern = [100];
    while (euc.is.alert >= 5) {
      buzzPattern.push(150, 500);
      euc.is.alert -= 5;
    }
    for (let i = 0; i < euc.is.alert; i++) buzzPattern.push(150, 150);
    buzzer.euc(buzzPattern);
    setTimeout(() => {
      euc.is.buzz = 0;
    }, 3000);
  }
...
// Function to reset temporary buffers
function resetTempBuffers() {
  euc.temp.tot = E.toUint8Array([0]);
  euc.temp.last = E.toUint8Array(euc.temp.tot.buffer);
}

// Function to check if a packet is valid based on start bytes
function isValidStartPacket(buffer) {
  return buffer.length > 4 && buffer[0] === 0xdc && buffer[1] === 0x5a && buffer[2] === 0x5c;
}

// Function to process the incoming packet
function processPacket(buffer) {
  ew.is.bt === 2 && console.log("Veteran: in: length:", buffer.length, " data :", [].map.call(buffer, (x) => x.toString(16)).toString());

  if (verifyChecksum(buffer)) {
    euc.temp.liveParse(buffer);
  } else {
    ew.is.bt === 2 && console.log("Packet checksum error. Dropped.");
  }
}
...
euc.temp.inpk = function (event) {
  ...
  // Check and concatenate packets based on start bytes
  ...
  if (euc.temp.tot.buffer.length < expectedLength) return;
  if (euc.temp.tot.buffer.length === expectedLength) processPacket(euc.temp.tot.buffer);
  else ew.is.bt === 2 && console.log("Packet size error. Dropped.");

  resetTempBuffers();
};

// Function to handle Bluetooth write operations
euc.wri = function (command) {
  if (euc.dbg) console.log("not connected yet");
  if (command === "end") euc.off();
};

// Function to connect to a device via Bluetooth
  ...
  NRF.connect(mac, { minInterval: 7.5, maxInterval: 15 })
    ...
    .then((c) => {
      ...
      euc.wri = function (cmd, value) {
        const handleWriteError = () => c.writeValue(value).catch(euc.off);
        ...
        switch (cmd) {
          ...
          case "hornOn":
            if (!euc.tout.horn) {
              euc.is.horn = 1;
              euc.tout.horn = setInterval(() => handleWriteError(euc.cmd("beep")), 150);
            }
            break;
          ...
          case "start":
            c.startNotifications()
              .then(() => (euc.dash.auto.onC.HL ? handleWriteError(euc.cmd(euc.dash.auto.onC.HL === 1 ? "setLightOn" : "setLightOff")) : null))
              .then(() => (euc.dash.auto.onC.clrM ? handleWriteError(euc.cmd("clearMeter")) : null))
              .then(() => (euc.dash.auto.onC.beep ? handleWriteError(euc.cmd("beep")) : null))
              .then(() => {
                euc.state = "READY";
                euc.is.run = 1;
              })
          ...
          case "end":
            Promise.resolve()
              .then(() => (euc.dash.auto.onD.HL ? handleWriteError(euc.cmd(euc.dash.auto.onD.HL === 1 ? "setLightOn" : "setLightOff")) : null))
              .then(() => (euc.dash.auto.onD.beep ? handleWriteError(euc.cmd("beep")) : null))
              .then(() => {
                euc.is.run = 0;
                euc.gatt.disconnect();
              })
          ...
          default:
            if (euc.cmd(cmd)) {
              euc.dbg && console.log("EUC module wri:", euc.cmd(cmd));
              handleWriteError(euc.cmd(cmd));
            }
        }
      };
