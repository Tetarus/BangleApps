...
function Layout(layout, options) {
  ...
  if (this.options.btns) {
    ...
    if (this.physBtns >= buttons.length) {
      ...
      if (this.physBtns > 2 && buttons.length == 1) buttons.unshift({ label: "" }); // pad so if we have a button in the middle
      while (this.physBtns > buttons.length) buttons.push({ label: "" });
      this._l.width = g.getWidth() - 8; // text width
      this._l = { type: "h", filly: 1, c: [this._l, { type: "v", pad: 1, filly: 1, c: buttons.map((b) => ((b.type = "txt"), (b.font = "6x8"), (b.height = btnHeight), (b.r = 1), b)) }] };
    } else {
      // add 'soft' buttons
      this._l.width = g.getWidth() - 32; // button width
      this._l = { type: "h", c: [this._l, { type: "v", c: buttons.map((b) => ((b.type = "btn"), (b.filly = 1), (b.width = 32), (b.r = 1), b)) }] };
...
Layout.prototype.setUI = function () {
  ...
  if (this.buttons) {
    ...
    Bangle.setUI({ mode: "updown", back: this.options.back, remove: this.options.remove }, (dir) => {
      ...
      if (dir === undefined && this.buttons[s]) return this.buttons[s].cb();
      if (this.buttons[s]) {
  ...
  if (this.b) {
    ...
    function pressHandler(btn, e) {
      if (e.time - e.lastTime > 0.75 && this.b[btn].cbl) this.b[btn].cbl(e);
      else if (this.b[btn].cb) this.b[btn].cb(e);
    }
...
Layout.prototype.render = function (l) {
  ...
  var cb = {
    ...
    txt: function (l) {
      ...
      if (l.wrap) {
        ...
        gfx
          .setFont(l.font)
          .setFontAlign(0, 0, l.r)
          .drawString(l.label, l.x + (l.w >> 1), l.y + (l.h >> 1));
      }
    },
    btn: function (l) {
      "ram";
      ...
      if (l.col !== undefined) gfx.setColor(l.col);
      if (l.src) gfx.setBgColor(btnface).drawImage("function" == typeof l.src ? l.src() : l.src, l.x + l.w / 2, l.y + l.h / 2, { scale: l.scale || undefined, rotate: Math.PI * 0.5 * (l.r || 0) });
      else
        gfx
          .setFont(l.font || "6x8:2")
          .setFontAlign(0, 0, l.r)
          .drawString(l.label, l.x + l.w / 2, l.y + l.h / 2);
    },
    img: function (l) {
      "ram";
      gfx.drawImage("function" == typeof l.src ? l.src() : l.src, l.x + l.w / 2, l.y + l.h / 2, { scale: l.scale || undefined, rotate: Math.PI * 0.5 * (l.r || 0) });
    },
    custom: function (l) {
      "ram";
      l.render(l);
  ...
  if (this.lazy) {
    ...
    var clearList = Object.keys(rectsToClear)
      .map((k) => rectsToClear[k])
      .reverse(); // Rects are cleared in reverse order so that the original bg color is restored
...
Layout.prototype.update = function () {
  ...
  var cb = {
    txt: function (l) {
      "ram";
      if (l.font.endsWith("%")) l.font = "Vector" + Math.round((gfx.getHeight() * l.font.slice(0, -1)) / 100);
      if (l.wrap) {
        l._h = l._w = 0;
      } else {
    ...
    btn: function (l) {
      "ram";
      if (l.font && l.font.endsWith("%")) l.font = "Vector" + Math.round((gfx.getHeight() * l.font.slice(0, -1)) / 100);
    ...
    img: function (l) {
      "ram";
      var m = gfx.imageMetrics("function" == typeof l.src ? l.src() : l.src),
