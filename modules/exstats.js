...
var state = {
  ...
  lastGPS: {},
  thisGPS: {}, // This & previous GPS readings
...
function formatTime(ms) {
  ...
  if (hrs === "0") return mins.padStart(2, 0) + ":" + secs.padStart(2, 0);
  else return hrs + ":" + mins.padStart(2, 0) + ":" + secs.padStart(2, 0); // dont pad hours
}
...
Bangle.on("GPS", function (fix) {
  ...
  if (state.lastGPS.fix) state.distance += calcDistance(state.lastGPS, fix);
...
if (Bangle.setBarometerPower)
  Bangle.on("pressure", function (e) {
    if (state.alt === undefined) state.alt = e.altitude;
    else state.alt = state.alt * 0.9 + e.altitude * 0.1;
...
exports.getStats = function (statIDs, options) {
  ...
  if (statIDs.includes("time")) {
    stats["time"] = {
      ...
      getString: function () {
        return formatTime(this.getValue());
      },
    };
  }
  if (statIDs.includes("dist")) {
    ...
    stats["dist"] = {
      ...
      getString: function () {
        return require("locale").distance(state.distance, 2);
      },
    };
  }
  if (statIDs.includes("step")) {
    stats["step"] = {
      title: "Steps",
      getValue: function () {
        return Bangle.getStepCount() - state.startSteps;
      },
      getString: function () {
  ...
  if (statIDs.includes("pacea")) {
    ...
    stats["pacea"] = {
      title: "A Pace",
      getValue: function () {
        return state.avrSpeed;
      }, // in m/sec
      getString: function () {
        return formatPace(state.avrSpeed, options.paceLength);
      },
    };
  }
  if (statIDs.includes("pacec")) {
    ...
    stats["pacec"] = {
      ...
      getString: function () {
        return formatPace(state.curSpeed, options.paceLength);
      },
    };
  }
  if (statIDs.includes("speed")) {
    ...
    stats["speed"] = {
      title: "Speed",
      getValue: function () {
        return state.curSpeed * 3.6;
      }, // in kph
      getString: function () {
        return require("locale").speed(state.curSpeed * 3.6, 2);
      },
    };
  }
  ...
  if (statIDs.includes("altg")) {
    ...
    stats["altg"] = {
      title: "Altitude",
      getValue: function () {
        return state.thisGPS.alt;
      },
      getString: function () {
        return state.thisGPS.alt === undefined ? "-" : Math.round(state.thisGPS.alt) + "m";
      },
    };
  }
  if (statIDs.includes("altb")) {
    ...
    stats["altb"] = {
      ...
      getString: function () {
        return state.alt === undefined ? "-" : state.alti + "m";
      },
    };
  }
  // ======================
  ...
  function reset() {
    ...
    state.notify = options.notify;
    if (state.notify.dist.increment > 0) state.notify.dist.next = state.distance + state.notify.dist.increment;
    if (state.notify.step.increment > 0) state.notify.step.next = state.startSteps + state.notify.step.increment;
    if (state.notify.time.increment > 0) state.notify.time.next = state.startTime + state.notify.time.increment;
  }
